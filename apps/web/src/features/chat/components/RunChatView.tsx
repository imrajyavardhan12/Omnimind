...
import type { ModelRunState } from '../api/runState'
...
export function RunChatView({ mode, className }: { mode: 'single' | 'compare'; className?: string }) {
  ...
  // history query. Once persisted, the panel is hidden and history renders it
  // (no double render). Match precisely by outputMessageId (from getRun), then
  // fall back to provider+model for completed runs so a failed getRun mapping
  // can't leave both the streamed buffer and the persisted message visible.
  const livePanels = useMemo(() => {
    const assistantMessages = messages.filter((m) => m.role === 'assistant')
    const byId = new Map(assistantMessages.map((m) => [m.id, m]))
    const claimed = new Set<string>()
    const panels: ModelRunState[] = []
    for (const id of run.state.order) {
      const mr = run.state.modelRuns[id]!
      if (mr.outputMessageId && byId.has(mr.outputMessageId)) {
        claimed.add(mr.outputMessageId)
        continue
      }
      if (mr.status === 'completed') {
        const match = assistantMessages.find(
          (m) => !claimed.has(m.id) && m.provider === mr.provider && m.model === mr.model,
        )
        if (match) {
  ...
  const handleSubmit = async (text: string) => {
    ...
    const request: CreateRunRequest = {
      ...
      models: selectedModels.map((m) => ({
        provider: m.provider,
        model: m.model,
        settings: { temperature, maxOutputTokens: maxTokens },
      })),
    }
